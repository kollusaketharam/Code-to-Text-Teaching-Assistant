...
// --- API Routes ---
app.post('${API_URL}/explain', async (req, res) => {
  const { code } = req.body;
...
app.post('${API_URL}/refactor', async (req, res) => {
  const { code } = req.body;
  if (!code) return res.status(400).json({ error: 'Code is required.' });
...
app.post('${API_URL}/debug', async (req, res) => {
  const { code } = req.body;
  if (!code) return res.status(400).json({ error: 'Code is required.' });
...
app.post('${API_URL}/translate', async (req, res) => {
